...
public class AfkCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        if(commandSender instanceof Player){
            ...
            Component registered = isregistered ? MiniMessage.miniMessage().deserialize(Rutils.getInstance().getConfig().getString("RegFormatOn")) : MiniMessage.miniMessage().deserialize(Rutils.getInstance().getConfig().getString("RegFormatOff"));

            if(args[0].equalsIgnoreCase("on")){

                String ChatMessageOnWithPlayerName = Rutils.getInstance().getConfig().getString("Chatmessageon", "").replace("%playername%", player.getName());
                ...
                Rutils.setPermission(player, "lu.afk", true);

            }else if(args[0].equalsIgnoreCase("off")){

                String ChatMessageOffWithPlayerName = Rutils.getInstance().getConfig().getString("Chatmessageoff", "").replace("%playername%", player.getName());
                ...
                Rutils.setPermission(player, "lu.afk", false);
